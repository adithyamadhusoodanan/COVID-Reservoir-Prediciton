setwd("~/COVID-Reservoir-Prediciton")
...
f1 <- subset(f1, select = -c(X, SeqName)) # remove 
...
vimps <-matrix(nrow=length(f_st3)-6-3, ncol=nloops) # create a m by n matrix where n number of rows (nloops) and m columns (number of features)
